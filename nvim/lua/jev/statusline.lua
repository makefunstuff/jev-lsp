...
--- makes — `jev.status` (PROTOCOL §6) — goes out in the background, at most once per
--- `POLL_MS`, and only while work is in flight. With no `jev` client attached it does
--- nothing at all.
---
--- @module 'jev.statusline'

local attach = require('jev.attach')

local M = {}

--- At most one `jev.status` poll per 5 s. `jev.status` is a cache read, not a model call,
...
--- The `jev.status` poll: its token, whether one is outstanding, and when the last one went
...
--- The `jev` client to poll: this buffer's, else any.
...
--- @param b table  `result.budget` from `jev.status`
...
local function render()
  ...
  local segment = ('jev: %d running'):format(count)
...
--- The segment. `''` when no `jev` client is attached, nothing is in flight, or the numbers
--- are not there yet.
---
...
--- request — that is the tracker in `jev` — only counted, so the segment answers "what is
--- running".
...
--- Fold a `jev.status` Result in (PROTOCOL §6/§7). Public because anything that already has
...
function M.install()
  local group = vim.api.nvim_create_augroup('jev.statusline', { clear = true })
  vim.api.nvim_create_autocmd('LspProgress', {
    group = group,
    pattern = '*',
    desc = 'jev: in-flight count and budget pressure for the statusline segment',
