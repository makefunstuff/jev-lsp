--- The action picker: `<leader>ja`'s flow, and the diff preview behind `preview = true`.
---
...
--- 4. `vim.lsp.util.apply_workspace_edit`, or `require('jev.diff')` when the caller asked for
---    a preview.
---
--- @module 'jev.picker'
...
--- The newest `$/progress` text from a `jev` client, and when it arrived.
local progress_message, progress_at = nil, 0

--- The plugin entry. Required lazily: `jev` requires this module, so a load-time require
--- would be a cycle.
--- @return table
...
--- The `jev` client for a buffer, if there is one.
--- @param bufnr integer
...
--- otherwise.
---
--- A selection is what `docs/UX.md` §2's `<leader>ja` means in visual mode, and the server
...
local function request_actions(c, bufnr, range, only, cb)
  ...
  local sent = c:request('textDocument/codeAction', {
    ...
    if err ~= nil then
      vim.notify(
        ('jev: code actions failed: %s'):format(err.message or vim.inspect(err)),
        vim.log.levels.WARN
      )
      cb({})
      return
...
---
--- @param items jev.PickerItem[]
--- @param on_choice fun(item: jev.PickerItem?)
...
function M.select(items, on_choice)
  ...
  if type(chooser) ~= 'function' then
    ...
    vim.notify(
      ('jev: no picker (vim.ui.select is unset); actions: %s'):format(table.concat(titles, ' | ')),
      vim.log.levels.INFO
    )
    return false
  end
  ...
  if not ok then
    vim.notify(('jev: the picker failed: %s'):format(err), vim.log.levels.ERROR)
    return false
  end
  return true
end
...
local function spin(token, label)
  ...
  timer:start(SPINNER_DELAY, SPINNER_TICK, vim.schedule_wrap(function()
    ...
    local text = progress_since(started) or ('jev: generating %s'):format(label)
...
--- @param c vim.lsp.Client?  the jev client, when one is attached
--- @param params table  `{ kind, verb?, line? }` (`jev.outcome`)
local function report(c, params)
  if c == nil then
    return
  end
  plugin().command('jev.outcome', { params }, function() end)
end
...
--- @param bufnr integer
--- @param action table
--- @param opts jev.PickerOpts
local function finish(c, bufnr, action, opts)
  if action.disabled ~= nil then
    M.reason(action.disabled.reason or ('jev: %s is unavailable'):format(action.title or 'the action'))
    return
  end
  ...
  vim.notify(
    ('jev: %s produced no change'):format(action.title or 'the action'),
    vim.log.levels.INFO
  )
end
...
local function run(c, bufnr, action, opts)
  ...
  if not c:supports_method('codeAction/resolve') then
    vim.notify(
      ('jev: %s resolved to nothing and the server cannot resolve it'):format(action.title or 'the action'),
      vim.log.levels.INFO
    )
    return
  end
  ...
  local provided = require('jev.context').for_position(bufnr, row[1] - 1, row[2])
...
function M.choose(c, bufnr, actions, opts)
  if #actions == 0 then
    vim.notify('jev: no code actions available', vim.log.levels.INFO)
    return
  end
  local items = {}
...
--- The `<leader>ja` flow. `M.action({ preview = true })` is the same flow with the resolved
...
--- Consume `$/progress` for the spinner, and keep the dedupe hook on every `jev` client.
---
--- Idempotent: one augroup, cleared and recreated; `watch_messages` refuses to wrap twice.
function M.install()
  local group = vim.api.nvim_create_augroup('jev.picker', { clear = true })
  vim.api.nvim_create_autocmd('LspProgress', {
    group = group,
    pattern = '*',
    desc = 'jev: what a resolve is doing, when the server says so',
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    group = group,
    desc = 'jev: report a resolve reason once, not once per channel',
    callback = function(ev)
