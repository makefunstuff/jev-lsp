...
--- The client's name, and the group the pass registers under. `:checkhealth meta` reads both.
M.NAME = 'meta'
M.AUGROUP = 'meta.attach'

--- @class meta.AttachOpts : vim.lsp.ClientConfig
--- @field cmd? string[]  Server command, default `{ 'meta-lsp' }`.
--- @field root_dir? string|fun(bufnr: integer, on_dir: fun(dir: string)): nil

--- Last options handed to `setup`, read by `configure`.
--- @type meta.AttachOpts
M.opts = {}
...
--- The kill switch (PROTOCOL §5, `:Meta stop`). `enabled = false` stops the whole server
--- (`docs/LANGUAGE.md` §7), so nothing is attached while it is off; `:Meta start` re-runs the
...
function M.is_enabled()
  ...
  return vim.tbl_get(settings or {}, 'meta', 'enabled') ~= false
end
...
function M.start()
  ...
  vim.api.nvim_create_autocmd({ 'BufReadPost', 'BufNewFile', 'BufWinEnter' }, {
    group = M.AUGROUP,
    desc = 'meta: attach to a file buffer the FileType path missed (docs/LANGUAGE.md §1)',
