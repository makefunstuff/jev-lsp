...
function M.propose(edit, opts)
  ...
  if target == nil then
    vim.notify('meta: nothing to preview — the edit touches no open buffer', vim.log.levels.WARN)
    return false
  end
  ...
  if orig_win == nil then
    vim.notify(
      ('meta: %s is in no window; open it to review the proposal')
  ...
  if not applied then
    ...
    vim.notify(('meta: the proposal does not apply cleanly: %s'):format(err), vim.log.levels.ERROR)
    return false
  end
  local after = vim.api.nvim_buf_get_lines(scratch, 0, -1, true)
  if vim.deep_equal(before, after) then
    vim.api.nvim_buf_delete(scratch, { force = true })
    vim.notify('meta: the proposal does not change this buffer', vim.log.levels.INFO)
    return false
  end
  ...
  pcall(vim.api.nvim_buf_set_name, scratch, ('meta://proposal/%s'):format(target.uri))
  ...
  both('n', '<CR>', approve, 'meta: apply this proposal')
  both('n', 'y', approve, 'meta: apply this proposal')
  both('n', 'q', reject, 'meta: reject this proposal')
  ...
  map(split_win, 'n', '<Esc>', reject, 'meta: reject this proposal')
