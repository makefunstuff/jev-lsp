...
**A client without the plugin gets a lens it cannot run, and that is stated rather than hidden.**
The namespace names who can act on the command, and the plugin is the only thing that can: a
client that is not the plugin runs the lens through its own stock path, which sends
`workspace/executeCommand` with `jev.plugin.pick` (or `jev.plugin.explain`) to the server — and
the server answers `{"ok": false, "error": {"code": "not_implemented", "message": "jev.plugin.pick
is not implemented in this version"}}`. Measured over the wire, not inferred. So outside Neovim
the lens is inert, with a structured refusal rather than silence. The alternative — a command id
the server itself serves — does not exist for this affordance: §7 leaves the server no way to open
the buffer an explanation is rendered into, which is the same reason `explain` is a command and
not a code action.
