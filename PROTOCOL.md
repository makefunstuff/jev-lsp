...
**`jev rules init` materialises the shipped set** (§9): one file per shipped file, under its
base name, in the format `.jev/rules/*.json` already uses, so that what a repository is inspected
with is a file it can open. It is idempotent and non-clobbering — a file that is already there is
