...
| 2026-09-21 | **A dismissal filters the push as well as the pull.** §9's two routes land in two namespaces — `nvim.lsp.jev.<client_id>` for `publishDiagnostics` and `nvim.lsp.jev.<client_id>.<pull_id>` for `textDocument/diagnostic` (`vim/lsp/diagnostic.lua`) — and the plugin registered its dismissal filter on the pull alone. `:Jev dismiss` therefore emptied the namespace the findings were in and the *next* background pass refilled it with the same finding under the same id, so `verify/dismiss_test.lua` and `verify/rules_live.lua` failed on both pinned Neovim versions against the run of 2026-09-21 (`dismiss[nvim-v0.12.5]`: "still 1", then "1 came back" after a fresh pull; the server log shows the pass landing between the two). The push route now goes through the same set (`filter_pushed`, sharing `without_dismissed` with `filter_findings`), and `hide_dismissed` clears every namespace the server's findings are in rather than the first. No server change, no weakening of the test: the claim §9 already made — a dismissal does not resurface — is what the row asserts. |
