...
//! cost gates, render the prompt `meta-core` owns, make one model call, validate the answer
...
use meta_core::budget::{Budget, Permit};
use meta_core::cache::{self, Cache, Conclusion};
use meta_core::config::{Config, TierConfig};
use meta_core::context;
use meta_core::contract;
use meta_core::document::{content_hash, Document};
use meta_core::edit::{self, BuildOptions};
use meta_core::findings;
use meta_core::gates;
use meta_core::lang;
use meta_core::model::{Backend, ChatRequest, ChatResponse};
use meta_core::plan;
...
use meta_core::verbs::{self, PromptSpec};
...
pub fn run(args: &[String], deps: &Deps) -> Outcome {
    match cli::parse(args) {
        Err(e) => Outcome {
            code: EXIT_USAGE,
            stdout: None,
            stderr: vec![format!("meta: {e}"), "meta: try `meta --help`".to_string()],
        },
        ...
        Ok(Invocation::Version) => Outcome {
            code: EXIT_OK,
            stdout: Some(format!("meta {}", meta_core::VERSION)),
            stderr: Vec::new(),
        },
...
fn settle(result: R<Value>, log: Vec<String>) -> Outcome {
    match result {
        ...
        Err(failure) => {
            let mut stderr = log;
            stderr.push(format!("meta: {}", failure.message()));
...
fn explain(config: &Config, target: &Target, deps: &Deps, log: &mut Vec<String>) -> R<Value> {
    ...
    let artifact = json!({
        ...
        // The same clock the server stamps artifacts with (meta-core::time).
...
fn review(config: &Config, target: &Target, deps: &Deps, log: &mut Vec<String>) -> R<Value> {
    let prepared = prepare(config, target, deps)?;
    let key = cache::findings_key(&prepared.doc.hash);
    ...
    let (body, from_cache) = match cached {
        ...
        None => {
            ...
            if built.plan.steps.is_empty() {
                ...
                // be located is not a step (meta-core::plan). Say so, rather than print an
...
fn prepare(config: &Config, target: &Target, deps: &Deps) -> R<Prepared> {
    ...
    let uri = match &target.source {
        ...
        // A document from stdin has no path. `meta-core` reads the language out of the
...
fn call(
    ...
    log.push(format!(
        "meta: model={} tier={} tokens_in={} tokens_out={} ms={ms} trigger=cli",
        tier.model,
        tier_name(tier_kind),
...
/// A `file://` URI, percent-encoded the way the LSP spells one. `meta-core`'s `path_from_uri`
...
mod tests {
    use super::*;
    use meta_core::document::path_from_uri;
    use meta_core::model::OpenAiCompat;
    use meta_core::types::Severity;
    ...
    fn status_needs_no_endpoint_and_still_succeeds() {
        ...
        let value = line(&out);
        assert_eq!(value["ok"], true);
        assert_eq!(value["version"], meta_core::VERSION);
    ...
    fn the_dedupe_gate_serves_a_cached_conclusion_without_calling_the_model() {
        ...
        cache.put(
            &cache::findings_key(&content_hash(RUST)),
            Conclusion {
    ...
    fn version_is_one_line() {
        ...
        assert_eq!(
            out.stdout.as_deref(),
            Some(format!("meta {}", meta_core::VERSION).as_str())
        );
    }

    #[test]
    fn the_file_uri_round_trips_through_the_parser_meta_core_uses() {
    ...
    fn temp_path(tag: &str) -> String {
        let dir = std::env::temp_dir().join(format!("meta-cli-{}-{tag}", std::process::id()));
