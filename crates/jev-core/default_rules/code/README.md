...
| a bare marker of unfinished work with no owner | `49` | `15` true. Whether a marker is tracked is a fact about the repository's convention (one repository names a scope in brackets after the marker, another names a plan item), and it is not on the line. Firing on a repository whose markers are its tracking gives noise and nothing else. The marker words themselves are left out of this file: `no-untracked-placeholders.json` claims `**/README.md`, so writing them here would make this README a candidate of the rule that table row describes. |
