...
| `bash editors/opencode/verify-bridge.sh` | `[opencode-bridge] 0 failure(s), 0 skip(s)` on this machine, four stages in 16 s: `opencode debug lsp diagnostics handler.rs` with `lsp.jev.command` pointing at the server (prints nothing for a flagged file) and at `editors/opencode/` (prints the finding at severity `1`), then `editors/opencode/opencode_probe.py` — a client written to OpenCode 1.18's behaviour — the same two ways, the native run required to record `pulls: 1, first_pull_items: 0, refresh_requests_acked: 2, pushes: 0`. Where `opencode` is not on `PATH` the client stages are `1 skip`, and the probe stages still run: `SKIP opencode is not on PATH`. Falsified deliberately, with the bridge replaced by a plain `exec` of the server: `3 failure(s)`, exit 1 |
...
the one row in this table that no runner invokes: it is a measurement taken by hand. The
`opencode_bridge` row and its harness are newer than that run (2026-09-21), so its figures above
are this machine's, not that run's; in CI its client stages report `1 skip` because `opencode` is
not installed on the runner, which is what the row says.
...
| Date | Event |
|---|---|
| 2026-09-21 | **OpenCode 1.18 got a client path, and it is a bridge, because the client does not finish the pull** (issue #21). Measured on 1.18: it advertises `workspace.diagnostics.refreshSupport: false` and answers the `workspace/diagnostic/refresh` the server sends anyway with an empty OK; it pulls `textDocument/diagnostic` once when a document opens, before the ambient pass has cached anything, and keeps that empty report for the session; and the `Diagnostic.report()` it hands the agent after a write keeps `severity === 1` only, so a rule finding (a Warning) reaches neither `opencode debug lsp diagnostics` nor the model. The product path stays pull (PROTOCOL §3.4/§9) — a server that pushed ambient findings would break §9 — so the adapter is a client one, `editors/opencode/jev-lsp-opencode-bridge.py`: it answers `initialize` with `diagnosticProvider` removed, tells the server `refreshSupport: true`, answers each refresh and pushes the pull to OpenCode as `textDocument/publishDiagnostics`, remaps Warning → Error with a `[jev warning]` prefix because Error is the only severity OpenCode's transcript shows, and synthesizes the `didSave` OpenCode never sends (the rules pass is save-triggered, so without it the pass never runs for OpenCode at all). `editors/opencode/verify-bridge.sh` is the row, and it checks both directions rather than one: `opencode debug lsp diagnostics handler.rs` on one fixture prints `[]` with `lsp.jev.command` pointing at the server and the finding at severity `1` behind the bridge, and `editors/opencode/opencode_probe.py` — a client written to OpenCode's four behaviours — has to record `pulls: 1, first_pull_items: 0, refresh_requests_acked: 2, pushes: 0` natively against `pushes: 2` behind the bridge. The stub decide tier is `verify/stub_model.py` with its decision call stalled 1.2 s, so the native pull lands before the finding exists instead of winning a race that a fast machine sometimes wins; `opencode` absent is a SKIP. Falsified deliberately: with the bridge replaced by a plain `exec` of the server the bridge stages report `3 failure(s)` and exit 1. `README.md`'s OpenCode section no longer recommends bare `jev-lsp --stdio`; `docs/VERIFICATION.md` §1.2 has the record. |
