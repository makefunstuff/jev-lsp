...
**Next action**: re-test the ambient path against a real one-shot client. Ambient findings are now
**pushed**, not only pulled (2026-09-21): a background pass ends by sending
`textDocument/publishDiagnostics` — replacing the `jev.checking…` cue it showed if the pass was
not sub-second — alongside the `workspace/diagnostic/refresh` the pull clients already use. That
closes the hole a one-shot client like omp had: a single `textDocument/diagnostic` at ~500 ms,
answered empty because no pass had landed yet, with nothing late ever reaching it. The clock is
documented and enforced: `ambient.pending_ms` (default 500) shows the cue, `ambient.budget_ms`
(default 8000) clears it and stops awaiting the pass, and both are keys a client can set. The
re-test notes (where to look, how to see the late publish and the cue) are in
`docs/research/omp-ambient-retest.md`; the automated assertions are `verify/rules_test.py` step 9
(a push carries the findings with no pull) and the `server::ambient_tests` unit tests. Nothing
else is open. **The rules have two sources: the repository's own and a set
...
| Date | Event |
|---|---|
| 2026-09-21 | **A dismissal now filters the push as well as the pull, which is where CI went red.** The ambient pass writes its conclusion with `textDocument/publishDiagnostics` and nvim renders a push into a different namespace from a pull (`nvim.lsp.jev.<client>.<pull_id>`, `vim/lsp/diagnostic.lua`), but the plugin registered the dismissal filter on `textDocument/diagnostic` alone (`nvim/lua/jev/init.lua`). So `:Jev dismiss` emptied the namespace the pushed findings were in, and the next background pass refilled it with the same finding under the same id: `verify/dismiss_test.lua` saw "still 1" and then "1 came back" after a fresh pull, and `verify/rules_live.lua` the same, on both pinned Neovim versions in run [35633114839](https://github.com/makefunstuff/jev-lsp/actions/runs/35633114839). The push route now goes through the same set (`filter_pushed`, sharing `without_dismissed` with `filter_findings`), and `hide_dismissed` clears every namespace the server's findings are in rather than the first one it found. No server change; the ambient push, the pending cue and the budget are untouched, and the two rows are unweakened — they assert exactly the §9 claim that was already there. |
| 2026-09-21 | **The ambient pass pushes its findings, and has a clock — the fix for the one-shot-client hole.** `publishDiagnostics` had been reserved for edits the server made (§9), so findings were pull-only and refreshed by `workspace/diagnostic/refresh`. A client that one-shots `textDocument/diagnostic` before its background pass has run — omp at ~500 ms — read the empty answer as "clean" and never saw the late result; a slow decide (~6 s measured) was a silent empty gutter in the meantime. Both are closed in `crates/jev-lsp/src/server.rs`: a finished pass now **pushes** what it concluded (it replaces a cue, or clears it on a no-hit) and still refreshes. `ambient.pending_ms` (default 500) pushes a document-level `jev.checking` information diagnostic when the pass is not sub-second; `ambient.budget_ms` (default 8000) is the hard ceiling that clears it and stops awaiting the pass, set above `models.decide.timeout_ms` (5000) so a healthy decision always lands. No severity remap and no `--opencode` costume. `verify/lsp_client.py` step 9's assertion is retargeted from "no push before the server's own change" to "no push for a document the server was never told about" (its `unsolicited_publish` defect now pushes to an unopened uri); `verify/rules_test.py` step 9 asserts the push carries the findings with no pull. Re-test notes: `docs/research/omp-ambient-retest.md`. |
| 2026-09-20 | **The rules gained a shipped source, because "no rules" was indistinguishable from "broken".** Measured: a repository with no `.jev/rules/` produced no ambient findings at all, and this project's own set published almost nothing on the repository it was built for. `crates/jev-core/default_rules/<group>/*.json` is embedded by a `build.rs` that globs it (`std::fs`, no new dependency; an empty tree compiles and behaves exactly as before, which is what lets two other sessions write the rule files in parallel), and `rules::load(root, rules.defaults, builtin)` merges it under the repository's own: a `.jev/rules/<id>.json` **shadows** the shipped rule with the same id, duplicate ids *within* a source are still kept and still linted, and `rules.defaults: false` is the old behaviour exactly. `jev rules init [--dir <dir>] [--force]` materialises the set to read and edit — each file named for its group (`prose-lists-end-in-etc.json`), so the two groups, authored in parallel by people who cannot see each other's file names, cannot collide; a within-group duplicate is still refused and nothing is written when that happens — and it is idempotent, non-clobbering, exit `2` with the refused names when it will not overwrite a file the user edited, writing nothing outside the target directory. Every finding carries `rule_source` (`repository` \| `builtin` \| `null` on a review finding), printed by `jev inspect` and by `:Jev inspect`; a pass carried by the shipped set says `default_rules` beside `no_rules`. The cache key is taken over the merged set, so the shipped rules are an input to every rules conclusion, and `noise.max_visible_findings` — which `findings_key` carried and `rules_key` forgot, so widening the cap served the old cap's findings — is in it now. The plugin has no `jobstart`/`vim.system` anywhere and does not know where the CLI binary is, so `rules init` is CLI-only: the plugin's every subcommand is either a server command or an action over data it already holds, and materialising the set from the client would be a second implementation of the writer. `verify/rules_test.py`'s fixtures now set `rules.defaults: false` (their counts are about the rules *they* write, and a shipped `.rs` rule would make every one of them wrong without anything being broken); the shipped path itself is asserted twice in the crate tests: over a fixture set (content-independent) and over the real embedded set (`jev::run::tests::a_repository_with_no_rules_is_inspected_by_the_shipped_set_this_binary_carries`, which derives its expectations — how many shipped rules claim `notes.md`, which titles may appear — from `builtin_files()` rather than pinning a count the shipped set owns, and then shadows one of them by id). |
